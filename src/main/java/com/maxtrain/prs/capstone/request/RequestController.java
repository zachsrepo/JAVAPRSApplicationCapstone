...
//	private final String Status_New = "NEW";
	...
	public ResponseEntity<Iterable<Request>> getRequestsInReview(@PathVariable int userId){
		Iterable<Request> requests = reqRepo.findByStatus(Status_Review);
		ArrayList<Request> notUserRequests = new ArrayList<Request>();
		for(Request rs : requests) {
			if(rs.getUser().getId() != userId) {
				notUserRequests.add(rs);
			}
		}
		
		return new ResponseEntity<Iterable<Request>>(notUserRequests, HttpStatus.OK);
